...
export default {
  ...
  onShow() {
    // 每次显示页面时刷新历史记录
    if (!this.imageUrl && !this.diagnosisResult) {
      this.loadDiagnosisHistory();
    }
  },
  methods: {
    ...
    async saveDiagnosis() {
      ...
      try {
        ...
        try {
          ...
          console.warn('图片上传失败，使用本地路径:', uploadError);
          cloudImageUrl = this.imageUrl;
        }
        
        // 调用云函数保存诊断记录
        const saveRes = await uniCloud.callFunction({
          name: 'saveDiagnosisRecord',
          data: {
        ...
        const diagnosisId = saveRes.result.data.diagnosisId;
        ...
        // 询问是否创建康复训练计划
        const stage = this.diagnosisResult.stage;
        setTimeout(() => {
          uni.showModal({
            title: '康复训练计划',
            content: `检测到您的骨关节炎处于${stage}期，系统可为您生成个性化康复训练计划，是否立即创建？`,
            confirmText: '立即创建',
            cancelText: '稍后再说',
            success: async (res) => {
              if (res.confirm) {
                await this.createRehabilitationPlan(diagnosisId, stage);
              } else {
                // 用户选择稍后，延迟重置诊断状态
                setTimeout(() => {
                  this.resetDiagnosis();
    ...
    async createRehabilitationPlan(diagnosisId, stage) {
      ...
      try {
        ...
        if (res.result.code === 0) {
          ...
          uni.showModal({
            title: '计划创建成功',
            content: `已为您生成${stage}期康复训练计划\n包含${data.videoCount}个训练视频\n建议每周训练${data.weeklyGoal}次`,
            confirmText: '去训练',
            cancelText: '稍后',
            success: (modalRes) => {
              if (modalRes.confirm) {
    ...
    async loadDiagnosisHistory() {
      ...
      try {
        ...
        if (res.result.code === 0) {
          // 格式化历史记录
          this.diagnosisHistory = res.result.data.records.map(record => ({
            id: record.id,
            type: record.diagnosisType,
            imageUrl: record.imageUrl,
            mainResult: record.mainResult,
            time: this.formatDateTime(record.createdAt),
            confidence: record.confidence,
            stage: record.stage,
            hasRehabPlan: record.hasRehabPlan
          }));
        }
